...
- `ADMIN_USERNAME` — логин для лёгкой админки (по умолчанию `admin`).
- `ADMIN_PASSWORD` — пароль для лёгкой админки (по умолчанию `admin`).
- `ADMIN_PATH` — путь для лёгкой админки (по умолчанию `control`).

## Лёгкая админка
Откройте `http://127.0.0.1:8000/<ADMIN_PATH>/` и войдите под логином/паролем из переменных окружения.
